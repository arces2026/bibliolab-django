...
7. [Importazione e Ripristino](#7-importazione-e-ripristino)
...
### 📝 **Creazione comando di esportazione**
...
### 📝 **Creazione comando di importazione**
...
# Escludi sessioni (temporanee)
python manage.py dumpdata --exclude sessions > dump.json

# Escludi migration records
python manage.py dumpdata --exclude django.migrations > dump.json
```
...
python manage.py dumpdata --exclude auth.permission --exclude contenttypes --exclude sessions > dump.json
```
...
|----------------|--------------|----------|-----------|
| **JSON (dumpdata)** | Migrazioni, condivisione dati | Indipendente DB, leggibile, relazioni mantenute | File grande, lento per DB grandi |
...
| **File (copia .sqlite3)** | Backup automatici notturni | Semplicissimo, veloce | Non modifica le relazioni, file binario |
| **CSV** | Export per analisi dati, Excel | Leggibile, usabile in altri software | Perde relazioni, tipo di dato |

### 📌 **Best Practices**

1. **Backup regolari**: ogni giorno o settimana
2. **Mantieni 3-5 versioni**: per poter tornare indietro
3. **Testa il ripristino**: verifica che i backup funzionino
4. **Usa versioni multiple**: JSON + copia del file
5. **Includi timestamp**: nei nomi dei file
6. **Escludi dati non necessari**: sessioni, permessi, ecc.
7. **Automazione**: usa cron o task scheduler
...
**Documentazione creata per progetto Django con SQLite3**  
